...
(function() {
    ...
    Storage.prototype.getItem = function getItem(name) {
        ...
        return this.data[sname] = value, this.fireEvent(sname, old, svalue), undefined
        ...
        return type === "object" && retVal && retVal._WB_wombat_obj_proxy ? (retVal instanceof Window && this.initNewWindowWombat(retVal), retVal._WB_wombat_obj_proxy) : retVal
        ...
        return function newSetLocProp(value) {
            ...
            if (this["_" + prop] !== value) {
                ...
                var rel = false;
                prop === "href" && typeof value === "string" && value && (value[0] === "." ? value = wombat.resolveRelUrl(value, this.ownerDocument) : value[0] === "/" && (value.length <= 1 || value[1] !== "/") && (rel = true, value = WB_wombat_location.origin + value));
                try {
        ...
        if (thisObj instanceof Window ? errorMsg = "Failed to construct '" + what + "': Please use the 'new' operator, this DOM object constructor cannot be called as a function." : args && args.length < needArgs && (errorMsg = "Failed to construct '" + what + "': " + needArgs + " argument required, but only 0 present."), errorMsg) throw new TypeError(errorMsg)
        ...
        if (url = url.replace("WB_wombat_", ""), mod === "if_" && this.wb_info.isSW && this.startsWith(url, "blob:")) return this.wb_info.prefix + this.wb_info.timestamp + "if_/" + url;
        ...
        if (this.startsWith(check_url, this.wb_replay_prefix) || this.startsWith(check_url, originalLoc.origin + this.wb_replay_prefix)) return url;
        if (originalLoc.host !== originalLoc.hostname && this.startsWith(url, originalLoc.protocol + "//" + originalLoc.hostname + "/")) return url.replace("/" + originalLoc.hostname + "/", "/" + originalLoc.host + "/");
        ...
        return prefix = this.startsWithOneOf(url, this.BAD_PREFIXES), prefix ? this.getFinalUrl(useRel, mod, this.extractOriginalURL(url)) : this.isHostUrl(url) && !this.startsWith(url, originalLoc.host + "/") ? this.getFinalUrl(useRel, mod, this.wb_orig_scheme + url) : url
        ...
        if (elem instanceof SVGElement) changed = this.rewriteSVGElem(elem);
            ...
            case "IFRAME":
                if (changed = this.rewriteFrameSrc(elem, "src"), this.wb_info.isSW && !changed) {
                    ...
                    if (srcdoc) elem.removeAttribute("srcdoc"), elem.src = this.wb_info.prefix + this.wb_info.timestamp + "id_/srcdoc:" + btoa(encodeURIComponent(srcdoc));
                    else {
                        var src = elem.getAttribute("src");
                        src && src !== "about:blank" || (!src && (elem.__WB_blank = true), elem.src = this.wb_info.prefix + this.wb_info.timestamp + "mp_/about:blank")
                    }
                }
        ...
        if (this.wb_info.presetCookie)
            for (var splitCookies = this.wb_info.presetCookie.split(";"), i = 0; i < splitCookies.length; i++) this.$wbwindow.document.cookie = splitCookies[i].trim()
        ...
        if (this.overridePropExtract(this.$wbwindow.XMLHttpRequest.prototype, "responseURL"), this.wb_info.isSW) {
            function jsonToQueryString(json) {
                ...
                try {
                    JSON.stringify(json, function(k, v) {
                        return ["object", "function"].includes(typeof v) || q.set(k, v), v
                    })
                } catch (e) {}
            ...
            function mfdToQueryString(mfd, contentType) {
                ...
                try {
                    for (var m, boundary = contentType.split("boundary=")[1], parts = mfd.split(new RegExp("-*" + boundary + "-*", "mi")), i = 0; i < parts.length; i++) m = parts[i].trim().match(/name="([^"]+)"\r\n\r\n(.*)/mi), m && params.set(m[1], m[2])
                } catch (e) {}
            ...
            this.$wbwindow.XMLHttpRequest.prototype.send = function(value) {
                if (this.__WB_xhr_open_arguments[0] === "POST") {
                    ...
                    typeof value === "string" && contentType === "application/x-www-form-urlencoded" || value instanceof URLSearchParams ? (this.__WB_xhr_open_arguments[0] = "GET", this.__WB_xhr_open_arguments[1] += (this.__WB_xhr_open_arguments[1].indexOf("?") > 0 ? "&" : "?") + value.toString(), value = null) : contentType === "application/json" || contentType === "text/plain" ? (this.__WB_xhr_open_arguments[0] = "GET", this.__WB_xhr_open_arguments[1] += (this.__WB_xhr_open_arguments[1].indexOf("?") > 0 ? "&" : "?") + jsonToQueryString(value), value = null) : wombat.startsWith(contentType, "multipart/form-data") && (this.__WB_xhr_open_arguments[0] = "GET", this.__WB_xhr_open_arguments[1] += (this.__WB_xhr_open_arguments[1].indexOf("?") > 0 ? "&" : "?") + mfdToQueryString(value, contentType))
                }
                if (this.__WB_xhr_open_arguments.length > 2 && (this.__WB_xhr_open_arguments[2] = true), this._no_rewrite || (this.__WB_xhr_open_arguments[1] = wombat.rewriteUrl(this.__WB_xhr_open_arguments[1])), origOpen.apply(this, this.__WB_xhr_open_arguments), !wombat.startsWith(this.__WB_xhr_open_arguments[1], "data:")) {
        ...
        if (this.$wbwindow.WebSocket && this.$wbwindow.WebSocket.prototype) {
            ...
            this.$wbwindow.WebSocket = function(WebSocket_) {
                return function WebSocket(url, configuration) {
                    ...
                    var rwURL = url;
                    return url != null && (rwURL = wombat.rewriteWSURL(url)), new WebSocket_(rwURL, configuration)
                }
            }(this.$wbwindow.WebSocket), this.$wbwindow.WebSocket.prototype = origWebSocket.prototype, Object.defineProperty(this.$wbwindow.WebSocket.prototype, "constructor", {
            ...
            if (ElementProto.setAttribute) {
                ...
                ElementProto._orig_setAttribute = orig_setAttribute, ElementProto.setAttribute = function setAttribute(name, value) {
                    ...
                    if (name && typeof rwValue === "string") {
                        ...
                        else if (lowername === "srcset") rwValue = wombat.rewriteSrcset(value, this);
                        else {
        ...
        if (win && !win._wb_wombat) {
            ...
            } else this.initProtoPmOrigin(win), this.initPostMessageOverride(win), this.initMessageEventOverride(win), this.initCheckThisFunc(win)
        }
        ...
        var local, session, pLocal = "localStorage",
        ...
        if (ThrowExceptions.yes = false, this.$wbwindow.Proxy) {
            var storageProxyHandler = function() {
                return {
                    ...
                    set: function(target, prop, value) {
                        return !target.hasOwnProperty(prop) && (target.setItem(prop, value), true)
                    },
            ...
            local = new this.$wbwindow.Proxy(new Storage(this, pLocal), storageProxyHandler()), session = new this.$wbwindow.Proxy(new Storage(this, pSession), storageProxyHandler())
        } else local = new Storage(this, pLocal), session = new Storage(this, pSession);
        this.defGetterProp(this.$wbwindow, pLocal, function localStorage() {
            ...
            return session
        }), this.$wbwindow.Storage = Storage, ThrowExceptions.yes = true
        ...
        var ownProps = this.getAllOwnProps($wbwindow),
            ...
            windowProxy = new $wbwindow.Proxy({}, {
                ...
                defineProperty: function(target, prop, desc) {
                    var ndesc = desc || {};
                    return ndesc.value || ndesc.get || (ndesc.value = $wbwindow[prop]), Reflect.defineProperty($wbwindow, prop, ndesc), Reflect.defineProperty(target, prop, ndesc)
                }
            });
        ...
        if (this.wb_is_proxy) return $wbwindow.__WB_replay_top = $wbwindow.top, void($wbwindow.__WB_top_frame = undefined);
